...
impl PlayScreen {
             ...
             content = content.push(
                container(
                    text(format!("Error: {}", error))
                        .color(Color::from_rgb(0.96, 0.47, 0.47))
                )
                .padding(10)
    ...
    fn view_hero(&self) -> Element<'_, Message> {
        let (name, version, _last_played) = if let Some(instance) = self.active_instance() {
            ...
            ("No Profile Selected".to_string(), "Create a profile to play".to_string(), "")
        };
        ...
        let launch_btn = button(
             row![
                 text(if self.is_launching { "Playing" } else { "Launch Game" })
                    .size(16)
                    .color(Color::WHITE)
             ]
             .spacing(8)
        ...
        .padding([12, 24])
        .on_press_maybe(if self.is_launching || self.active_instance().is_none() { None } else { Some(Message::Launch) })
        .style(|_theme, status| {
        ...
        let list: Element<'_, Message> = if self.instances.is_empty() {
            ...
            Element::from(column(
                self.instances.iter().map(|inst| {
                    ...
                    let play_btn = button(
                        text(btn_text).size(14).color(Color::WHITE)
                    )
                    .padding(10)
                        ...
                        let disabled = Color::from_rgb(0.3, 0.3, 0.3);
                         ...
                         iced::widget::button::Style {
                            background: Some(match status {
                                ...
                                _ => if is_launching && !is_active { disabled } else { base },
                            }.into()),
                            border: iced::Border {
                    ...
                    // Settings cog
                    let settings_btn = button(text("⚙").size(14).color(Color::from_rgb(0.7,0.7,0.7)))
                        .padding(10)
                    ...
                    button(content)
                        .on_press_maybe(if is_launching { None } else { Some(Message::SelectInstance(inst.id.clone())) })
                        .padding(12)
                        .width(Length::Fill)
                        .style(move |_theme, status| {
                             let bg = match status {
                                 iced::widget::button::Status::Hovered => Color::from_rgb(0.15, 0.15, 0.16),
                                 _ => bg_color,
                             };
                             ...
                             let opacity_factor = if is_launching && !is_active { 0.5 } else { 1.0 };
                             ...
                             let mut style = iced::widget::button::Style {
                                 ...
                                 text_color: Color { a: opacity_factor, ..Color::WHITE }, 
                                 ..iced::widget::button::Style::default()
                             };
                             ...
                             if is_launching && !is_active {
                                if let Some(iced::Background::Color(c)) = style.background {
                                    style.background = Some(iced::Background::Color(Color { a: 0.5, ..c }));
                                }
                             }
