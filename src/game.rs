...
pub fn prepare_and_launch(
    ...
    let version_data: VersionData = {
        ...
        let fetch_manifest = || -> Result<VersionData, String> {
            ...
            if !resp.status().is_success() {
                 return Err(format!("Failed to fetch main manifest: Status {}", resp.status()));
            }
            let manifest_json: serde_json::Value = resp.json().map_err(|e| format!("Failed to parse main manifest: {}", e))?;
            ...
            if !resp.status().is_success() {
                 return Err(format!("Failed to fetch 1.21 manifest: Status {}", resp.status()));
            }
            ...
            serde_json::from_str(&content).map_err(|e| format!("Failed to parse downloaded manifest: {}", e))
        };
            ...
            match fs::read_to_string(&version_json_path) {
                Ok(content) => {
                    match serde_json::from_str::<VersionData>(&content) {
                        Ok(data) => data,
                        Err(e) => {
                            println!("Local manifest corrupted ({}). Deleting and re-downloading...", e);
        ...
        if let Some(classifiers) = lib.downloads.classifiers {
             ...
             if let Some(native_obj) = classifiers.get("natives-windows") {
                 ...
                 if let Ok(file_info) = serde_json::from_value::<DownloadFile>(native_obj.clone()) {
                     // Download native jar
                     let nat_path = libraries_dir.join(format!("{}-native.jar", lib.name.replace(':', "-")));
                     if !nat_path.exists() {
    ...
    let asset_index_path = assets_dir.join("indexes").join(format!("{}.json", version_data.assetIndex.id));
    ...
    let index_data: serde_json::Value = serde_json::from_str(&index_content).map_err(|e| e.to_string())?;
    ...
    let config = VanillaLaunchConfig {
        ...
        memory: Some(MemorySettings { min_megabytes: 1024, max_megabytes: 4096 }),
        extra_jvm_args: vec![],
        extra_game_args: vec![],
    ...
    let auth = match &account.kind {
        account_manager::AccountKind::Microsoft { uuid, username } => {
        ...
        account_manager::AccountKind::Offline { username, uuid } => {
            LaunchAuth::Offline { username: username.clone(), uuid: uuid.clone() }
        }
    };
...
fn download_file(url: &str, path: &Path) -> Result<(), String> {
    ...
    let mut response = reqwest::blocking::get(url).map_err(|e| format!("Failed to GET {}: {}", url, e))?;
    
    if !response.status().is_success() {
        return Err(format!("Download failed for {}: Status {}", url, response.status()));
    }

    let mut file = fs::File::create(path).map_err(|e| format!("Failed to create file {:?}: {}", path, e))?;
    io::copy(&mut response, &mut file).map_err(|e| format!("Failed to write to {:?}: {}", path, e))?;
    Ok(())
}
...
fn maven_to_path(maven_id: &str) -> PathBuf {
    ...
    let name = parts[1];
    let version = parts[2];
    PathBuf::from(format!("{}/{}/{}/{}-{}.jar", domain, name, version, name, version))
}
